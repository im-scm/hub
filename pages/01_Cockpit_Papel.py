...
.cp-kpi-label {font-size:.78rem; color:#6B7280; margin-bottom:6px;}.cp-kpi-value {font-size:1.14rem; font-weight:700; color:#111827; line-height:1.2;}
.cp-table-shell {border:1px solid #E5E7EB; border-radius:14px; background:#FFF; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,.03);}.cp-table-wrap {overflow-y:auto; overflow-x:auto;}
.cp-table {width:100%; border-collapse:separate; border-spacing:0; table-layout:auto; font-size:.84rem; color:#111827;}.cp-table thead th {position:sticky; top:0; z-index:2; background:#F9FAFB; color:#6B7280; font-weight:600; border-bottom:1px solid #E5E7EB; border-right:1px solid #F0F2F5; padding:8px 10px; white-space:nowrap;}
.cp-table tbody td {padding:7px 10px; border-bottom:1px solid #F3F4F6; border-right:1px solid #F8FAFC; white-space:nowrap; background:#FFF;}.cp-table tbody tr:hover td {background:#FAFBFF;}
...
def detect_header_row(raw):
    ...
    for i in range(min(len(raw), 50)):
        ...
        score = sum(t in row_text for t in targets)
        if score > score_best:
...
def build_html_table(df_html, height_px=560):
    if df_html.empty:
        return "<div class='cp-note'>Sem registros para exibir.</div>"
...
def build_canonical(df):
    aliases = {
        "Material Number": ["Material Number", "Material"], "Impress Type": ["Impress Type", "Print Type"], "Width (mm)": ["Width (mm)", "Width"],
        "g/m2": ["g/m2", "g/m²", "Gramatura"], "Supplier": ["Supplier", "Fornecedor"], "Currency": ["Currency", "Moeda"],
        "Current Price": ["Current Price", "Preço Atual", "Preco Atual"], "Lot (ton)": ["Lot (ton)", "Lot", "Lote"],
        "TCO (R$/KG)": ["TCO (R$/KG)", "TCO"], "TCO (R$/M2)": ["TCO (R$/M2)"], "Payment Terms": ["Payment Terms", "Prazo Pagamento"],
        "P.Value (R$/KG)": ["P.Value (R$/KG)", "P.Value"], "P.Value (R$/M2)": ["P.Value (R$/M2)"],
        "Última Atualização de Preço": ["Última Atualização de Preço", "Ultima Atualizacao de Preco", "Last Update"],
    }
    rename = {orig: k for k, a in aliases.items() if (orig := find_column(df.columns, a))}
    d = df.rename(columns=rename).copy()
    d = d[[c for c in aliases.keys() if c in d.columns]].copy()
    for c in ["Width (mm)", "g/m2", "Current Price", "Lot (ton)", "TCO (R$/KG)", "TCO (R$/M2)", "P.Value (R$/KG)", "P.Value (R$/M2)", "Payment Terms"]:
        if c in d.columns: d[c] = d[c].apply(parse_number)
    if "Última Atualização de Preço" in d.columns:
        d["Última Atualização de Preço"] = pd.to_datetime(d["Última Atualização de Preço"], errors="coerce", dayfirst=True)
...
def load_data_with_audit():
    ...
    if h is None:
        raise ValueError("Não foi possível identificar o cabeçalho da aba 'Preços e Condições'.")
    raw2 = pd.read_excel(EXCEL_FILE, sheet_name=SOURCE_SHEET, header=h, engine="openpyxl")
    raw2.columns = [re.sub(r"\s+", " ", str(c).strip().replace("\n", " ")) for c in raw2.columns]
    raw2 = raw2.dropna(how="all")
    final = build_canonical(raw2)
    return final, {"original": len(raw2), "final": len(final), "removed": max(len(raw2)-len(final), 0)}


def create_filter(data, col, label, numeric=False):
    if col not in data.columns: return data
    vals = data[col].dropna().unique().tolist()
    vals = sorted(vals) if numeric else sorted([str(v) for v in vals])
    selected = st.sidebar.multiselect(label, vals, default=[])
    if not selected: return data
    return data[data[col].isin(selected)] if numeric else data[data[col].astype(str).isin(selected)]
...
with st.sidebar:
    st.divider()
    st.markdown("### Filtros | Cockpit Papel")
